...
class YaraEngine(threading.Thread):
    ...
    def __init__(self, spidie):
        ...
        self.logger.info("Initialising YaraEngine")
    ...
    def init_rules(self):
        ...
        if not os.path.exists(r):
            self.logger.error("Default yara rule file {}: not found".format(r))
            return
    ...
    def run(self):
        if not self.rules:
            self.logger.debug("Skipping YaraEngine scane since we have no rules")
            return

        self.logger.info("Starting YaraEngine")
    ...
    def scan(self, rawdata):
        ...
        if self.isBase64(rawdata):
            try:
                ...
                self.logger.info("Could not decode")
